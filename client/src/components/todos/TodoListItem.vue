...
<template>
  <div class="group bg-white rounded-xl shadow-sm p-4 flex items-center justify-between
           hover:shadow-md transition-shadow duration-200">
    <div class="flex items-center gap-4">
      <div class="relative">
        <input type="checkbox" :checked="todo.completed" @change="emit('toggle')" class="peer h-5 w-5 rounded border-slate-300 text-violet-600
                 focus:ring-4 focus:ring-violet-500/20" />
        <CheckIcon class="absolute top-0.5 left-0.5 h-4 w-4 text-white
    ...
    <button @click="emit('remove')" class="text-slate-400 hover:text-red-500 focus:outline-none
